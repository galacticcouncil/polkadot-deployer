...
class Helm {
  ...
  async install(){
    ...
    const chartName = '/home/frido/Projects/Hydradx/polkadot-charts/charts/polkadot';
    console.warn('remove this before pr/commit');
  ...
  _generateValuesFile(index) {
    ...
    if (this.config.imagePullSecret) {
      values.imagePullSecret = {};
      values.imagePullSecret.secretName = this.config.imagePullSecret.secretName;
      if (!this.config.imagePullSecret.imageCredentials) {
        throw new Error('imagePullSecret - imageCredentials proprety is required');
      }
      values.imagePullSecret.imageCredentials = {
        registry: this.config.imagePullSecret.imageCredentials.registry,
        username: this.config.imagePullSecret.imageCredentials.username,
        password: this.config.imagePullSecret.imageCredentials.password
      };
    }
